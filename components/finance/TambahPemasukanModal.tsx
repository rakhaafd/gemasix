...
interface TambahPemasukanModalProps {
  isOpen: boolean;
  onClose: () => void;
  initialData?: FinanceTransaction | null;
}

export default function TambahPemasukanModal({ isOpen, onClose, initialData }: TambahPemasukanModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showSuccess("Berhasil!", initialData?.id ? "Pemasukan berhasil diperbarui." : "Pemasukan baru berhasil ditambahkan.");
      onClose();
    } catch (error) {
      console.error("Failed to save income", error);
      showError("Gagal!", "Gagal menyimpan pemasukan. Silakan coba lagi.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <ModalWrapper isOpen={isOpen} onClose={onClose} title={initialData ? "Edit Pemasukan" : "Tambah Pemasukan"}>
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        <div>
          <FormLabel htmlFor="title" required>Judul Pemasukan</FormLabel>
          <FormInput
            id="title"
            type="text"
            required
            ...
            placeholder="Contoh: Iuran Warga Bulan Agustus"
          />
        </div>
        ...
        <div>
          ...
          <FormTextarea
            ...
            placeholder="Tambahkan detail pemasukan jika diperlukan..."
          />
        </div>

        <div>
          <FormLabel htmlFor="image">Gambar / Bukti (Opsional)</FormLabel>
